...
def upload_attachment(browser, attachment_path, headless=False):
    ...
    print("before click vedhaeft nyt")
    ...
    if headless:
        ...
        print("headless upload via send_keys")
...
def fill_out_form_and_control(browser, item_data):
    ...
    # ---------------------------------------------------------
    print("before switching to frames")

    browser.switch_to.default_content()
    switch_to_frame(browser, "contentAreaFrame")
    switch_to_frame(browser, "ivuFrm_page0ivu0")

    print("after switching to frames")

    # ---------------------------------------------------------
    # 2. FOCUS FIRST ROW IN POSTING TABLE
    # ---------------------------------------------------------
    print("before focus first row")
    ...
    # ---------------------------------------------------------
    # 3. FILL POSTING LINE USING ACTIVE ELEMENT (SAP SAFE)
    # ---------------------------------------------------------
    ...
    print("after scroll into view")

    time.sleep(1)

    browser.execute_script("arguments[0].click();", kontroller)

    print("after clicking kontroller")

    # ---------------------------------------------------------
    # 6. WAIT FOR VALIDATION RESULT
    # ---------------------------------------------------------
    print("before wait for validation")
    ...
    print("after wait for validation")
